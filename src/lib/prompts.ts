...
- ⚠️ 말 끊김 방지 (절대 준수): source_clip의 타임스탬프는 연예인의 대사 첫 마디 최소 0.2초 전 시작(sourceStartMs)하고 마지막 음절이 끝나고 최소 0.5초 뒤 종료(sourceEndMs)로 설정하여, 대사가 도중에 뚝 끊기는 현상을 완전히 차단하십시오.
- ⚠️ 기승전결 및 마무리 여운 (초강력 결말 규칙): 흐름이 갑자기 끊어져 "잉?" 하고 허무하게 끝나지 않도록 결론을 매듭지어야 합니다. 마지막 세그먼트는 반드시 연예인의 유쾌한 끝인사, 털털한 웃음소리, 위트 있는 다짐 멘트, 혹은 마무리 인사(source_clip) 등 여운 있는 퇴장 장면으로 채워야 하며, 제품 설명 도중에 끊어버리는 대본 구성을 절대 금지합니다.
...
- [제품 근거 JSON]에 적힌 타임스탬프 부근(출연자가 해당 제품을 먹거나 사용/언급하는 부근)을 참고하여 제품과 관련된 3~6초 진짜 음성 타임스탬프(sourceStartMs~sourceEndMs)를 직접 정밀 추출하십시오. (단, 말 끊김이 없도록 음절 앞뒤 여유를 충분히 고려하여 시작과 끝을 설정할 것)
