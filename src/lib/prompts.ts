...
export const makeTranscriptPrompt = async (): Promise<string> => {
  ...
  return `${base}\n\n[API JSON 출력 계약 — 절대 준수]\n- Markdown이나 설명을 출력하지 말고 제공된 JSON Schema만 출력한다.\n- title은 내부 식별용 짧은 제목이다.\n- hookTitle은 영상 상단에 표시할 후킹 제목이다. 각 줄당 5~8자 이내로 극도로 임팩트 있고 짧게 작성한다 (반드시 2줄 이내, 실제 개행 문자로 줄바꿈).\n- ⚠️ hookTitle 규칙: 첫 줄에는 연예인 이름과 핵심 키워드를 적극 활용하라 (예: "이국주 찐애정템", "소유 10년 내돈내산", "강민경 최애 꿀템"). 둘째 줄에는 궁금증을 유도하는 짧은 문구를 써라 (예: "대체 뭐길래?", "이건 못참지", "난리난 이유").\n- hookTitle은 과장된 의학적 효능이나 확인되지 않은 사실을 단정하지 않는다.\n- scriptText에는 전체 대본을 사람이 읽을 수 있는 문자열로 넣는다.\n- segments는 narration 또는 source_clip 배열이다.\n- narration에는 text만 넣으며, 나레이션은 최소화하여 1문장 이내의 짧은 징검다리 역할만 수행한다. narration의 text는 절대로 2문장 이상을 합쳐 쓰지 말고 1문장(15자 이내) 단위로 짧게 나누어 작성하라.\n- 영상 전체에서 연예인의 실제 발언(source_clip) 비중이 60% 이상이어야 한다.\n- ⚠️ source_clip 선정 필수 규칙: source_clip은 반드시 입력된 승인 [제품/아이템]과 직접 관련된 발언·먹방·시연·리액션 구간이어야 한다. 핸드폰/문자 이야기나 제품과 무관한 엉뚱한 잡담을 source_clip으로 선택하지 마라.\n- [제품 근거 JSON]에 적힌 타임스탬프 부근(출연자가 해당 제품을 먹거나 사용/언급하는 부근)을 참고하여 제품과 관련된 3~6초 진짜 음성 타임스탬프(sourceStartMs~sourceEndMs)를 직접 정밀 추출하라.\n- sourceEndMs는 sourceStartMs보다 반드시 커야 하며, 자막(subtitle) 문장 끝에는 마침표(.)를 붙이지 않는다.`;
};
