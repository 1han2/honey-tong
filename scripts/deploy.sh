...
# Load Supertone configs from .env if present
if [ -f .env ]; then
  SUPERTONE_API_KEY="${SUPERTONE_API_KEY:-$(grep -E "^SUPERTONE_API_KEY[[:space:]]*=" .env | sed -E "s/^SUPERTONE_API_KEY[[:space:]]*=[[:space:]]*['\"]?([^'\"]*)['\"]?/\1/" | xargs)}"
  SUPERTONE_VOICE_ID="${SUPERTONE_VOICE_ID:-$(grep -E "^SUPERTONE_VOICE_ID[[:space:]]*=" .env | sed -E "s/^SUPERTONE_VOICE_ID[[:space:]]*=[[:space:]]*['\"]?([^'\"]*)['\"]?/\1/" | xargs)}"
fi
